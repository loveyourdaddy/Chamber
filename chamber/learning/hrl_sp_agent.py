...
import chamber.ase.hrl_agent as hrl_agent
from chamber.utils.utils import load_check, load_checkpoint
...
class HRLSPAgent(hrl_agent.HRLAgent):
    ...
    def play_steps(self):
        ...
        for n in range(self.horizon_length):
            ...
            obs = torch.cat((self.obs['obs'], self.obs['obs_op']), dim=0) # op와 합쳐서 업데이트
            # self.experience_buffer.update_data('obses', n, self.obs['obs'])
            self.experience_buffer.update_data('obses', n, obs) 
            ...
            for k in update_list:
                ...
                if hasattr(res_dict_op, k): 
                    ...
                    tmp = torch.cat((res_dict[k], res_dict[k]))
                self.experience_buffer.update_data(k, n, tmp)
            ...
            # concat
            self.obs['obs'] = torch.cat((self.obs['obs'], self.obs['obs_op']), dim=0) # op와 합쳐서 업데이트
            shaped_rewards = torch.cat((shaped_rewards, shaped_rewards), dim=0) # op와 합쳐서 업데이트
            dones = torch.cat((self.dones, self.dones), dim=0) # op와 합쳐서 업데이트
            disc_rewards = torch.cat((infos['disc_rewards'], infos['disc_rewards']), dim=0) # op와 합쳐서 업데이트
            ...
            # concat 
            terminated = torch.cat((terminated, terminated), dim=0) # op와 합쳐서 업데이트
            
            next_vals = self._eval_critic(self.obs)
            # next_vals = self._eval_critic(obs) # critic 
        ...
        while True:
            ...
            if self.rank == 0:
                ...
                if self.game_rewards.current_size > 0:
                    ...
                    print("epoch:", epoch_num, "mean_rewards:", mean_rewards, "mean_lengths:", mean_lengths)
